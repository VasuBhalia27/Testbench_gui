...
class AutomationGUI:
    def __init__(self, parent_widget=None, lock_callback=None, unlock_callback=None):
        ...
        # Debugger backend selection (persists across runs)
        self.debugger_backend = tk.StringVar(master=self.root, value="trace32")
    ...
    def _build_ui(self, container):
        ...
        # Debugger backend selector (Trace32 / J-Link) — always visible before Start
        self.dbg_frame = ttk.Labelframe(btn_frame, text="Debugger Backend")
        self.dbg_frame.pack(side="left", padx=(16, 0))

        self.dbg_trace32_rb = ttk.Radiobutton(
            self.dbg_frame,
            text="Trace32",
            variable=self.debugger_backend,
            value="trace32",
        )
        self.dbg_trace32_rb.grid(row=0, column=0, sticky="w", padx=5, pady=3)

        self.dbg_jlink_rb = ttk.Radiobutton(
            self.dbg_frame,
            text="J-Link",
            variable=self.debugger_backend,
            value="jlink",
        )
        self.dbg_jlink_rb.grid(row=1, column=0, sticky="w", padx=5, pady=3)
    ...
    def _on_end(self) -> None:
        ...
        try:
            ...
            self.append_status("Closing debugger (force cleanup if needed)...")
            ...
            self.append_status("Debugger closed.")
        ...
        self.dbg_trace32_rb.config(state="normal")
        self.dbg_jlink_rb.config(state="normal")
        self.variant.set(0)
        self.non_nfc_cb.state(["!selected"])
        self.nfc_cb.state(["!selected"])
    ...
    def _on_start(self):
        ...
        self.dbg_trace32_rb.config(state="disabled")
        self.dbg_jlink_rb.config(state="disabled")
        # Apply selected debugger backend before connecting
        selected_backend = self.debugger_backend.get()
        try:
            from Functional import debugger as t32
            t32.set_backend(selected_backend)
        except Exception as _be:
            self.append_status(f"Warning: could not set debugger backend: {_be}")
    ...
    def show_restart_warning(self) -> None:
        ...
        try:
            ...
            self.append_status(f"Note: debugger close: {e}")

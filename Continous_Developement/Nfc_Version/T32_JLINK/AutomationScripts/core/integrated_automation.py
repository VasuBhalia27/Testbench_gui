...
class IntegratedAutomationRunner:
    ...
    def _run_automation_thread(self, variant: int) -> None:
        ...
        try:
            ...
            # determine whether the active backend already has an open connection
            already_connected = t32.is_connected()
            if already_connected:
                self._log(f"Using existing {t32.get_backend_name()} connection")
            else:
                self._log(f"No {t32.get_backend_name()} connection detected; will connect now")

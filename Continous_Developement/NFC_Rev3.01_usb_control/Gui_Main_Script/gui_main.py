...
# CAPA relay helper state
capa_relay_port = tk.StringVar(value="COM3")
...
canvas8.create_text(570.0, 138.0, anchor="nw", text="USB Relay", fill="#FFFFFF", font=("Inter SemiBold", 15 * -1))
canvas8.create_text(560.0, 168.0, anchor="nw", text="Port", fill="#FFFFFF", font=("Inter SemiBold", 12 * -1))
tab8_relay_port_entry = ttk.Entry(tab8_frame, textvariable=usb_relay_port, style='Background_grey.TEntry')
tab8_relay_port_entry.place(x=610.0, y=168.0, width=90, height=28)
canvas8.create_text(560.0, 210.0, anchor="nw", text="Baud", fill="#FFFFFF", font=("Inter SemiBold", 12 * -1))
tab8_relay_baud_entry = ttk.Entry(tab8_frame, textvariable=usb_relay_baud, style='Background_grey.TEntry')
tab8_relay_baud_entry.place(x=610.0, y=210.0, width=90, height=28)
canvas8.create_text(560.0, 252.0, anchor="nw", text="Short relay", fill="#FFFFFF", font=("Inter SemiBold", 12 * -1))
tab8_relay_short_entry = ttk.Entry(tab8_frame, textvariable=usb_relay_short_relay, style='Background_grey.TEntry')
tab8_relay_short_entry.place(x=660.0, y=252.0, width=40, height=28)
canvas8.create_text(560.0, 294.0, anchor="nw", text="Reset relay", fill="#FFFFFF", font=("Inter SemiBold", 12 * -1))
tab8_relay_reset_entry = ttk.Entry(tab8_frame, textvariable=usb_relay_reset_relay, style='Background_grey.TEntry')
tab8_relay_reset_entry.place(x=660.0, y=294.0, width=40, height=28)
canvas8.create_text(560.0, 336.0, anchor="nw", text="Pulse ms", fill="#FFFFFF", font=("Inter SemiBold", 12 * -1))
tab8_relay_duration_entry = ttk.Entry(tab8_frame, textvariable=usb_relay_duration_ms, style='Background_grey.TEntry')
tab8_relay_duration_entry.place(x=660.0, y=336.0, width=40, height=28)

relay_status_label = tk.Label(tab8_frame, textvariable=relay_status_text, bg="#DFDFDF", font=("Inter", 10))
relay_status_label.place(x=560, y=376, width=190, height=24)

relay_connect_btn = ttk.Button(tab8, text="Connect Relay", command=lambda: connect_capa_relay())
...
def _update_relay_status(message: str) -> None:
    relay_status_text.set(message)


def connect_capa_relay() -> None:
    global capa_relay_controller
    try:
        capa_relay_controller = USBRelayController(
            port=usb_relay_port.get(),
            baudrate=usb_relay_baud.get(),
            relay_short_capa=usb_relay_short_relay.get(),
            relay_reset_switch=usb_relay_reset_relay.get(),
        )
        capa_relay_controller.open()
        _update_relay_status(f"Relay: connected {usb_relay_port.get()}")
    except Exception as exc:
        capa_relay_controller = None
        _update_relay_status("Relay: disconnected")
        messagebox.showerror("Relay Error", f"Failed to connect to USB relay: {exc}")


def disconnect_capa_relay() -> None:
    global capa_relay_controller
    if capa_relay_controller is None:
        return
    try:
        capa_relay_controller.close()
    except Exception as exc:
        messagebox.showerror("Relay Error", f"Failed to disconnect USB relay: {exc}")
    finally:
        capa_relay_controller = None
        _update_relay_status("Relay: disconnected")


def _pulse_relay(relay_id: int, duration_ms: int) -> None:
    if capa_relay_controller is None:
        messagebox.showerror("Relay Error", "USB relay is not connected.")
        return

    try:
        capa_relay_controller.pulse_relay(relay_id, duration_ms / 1000.0)
    except Exception as exc:
        messagebox.showerror("Relay Error", f"Failed to pulse relay {relay_id}: {exc}")


def short_capa_via_relay() -> None:
    _pulse_relay(usb_relay_short_relay.get(), usb_relay_duration_ms.get())


def reset_capa_switch_via_relay() -> None:
    _pulse_relay(usb_relay_reset_relay.get(), usb_relay_duration_ms.get())

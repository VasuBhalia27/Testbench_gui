...
class TestSequenceRunner:
    ...
    def run_capa_test(self) -> dict:
        ...
        if self._relay_controller is not None:
            self._log("External relay attached: preparing CAPA hardware before measurement")
            try:
                self._relay_controller.reset_switch(duration=0.2)
                time.sleep(0.25)
                self._relay_controller.short_capa(duration=0.5)
                time.sleep(0.25)
            except Exception as exc:
                self._log(f"Warning: external CAPA relay preparation failed: {exc}")

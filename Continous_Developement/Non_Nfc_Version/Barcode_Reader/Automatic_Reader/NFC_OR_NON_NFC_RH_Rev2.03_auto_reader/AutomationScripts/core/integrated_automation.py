...
class IntegratedAutomationRunner:
    ...
    def _run_automation_thread(self, variant: int) -> None:
        ...
        try:
            ...
            from AutomationScripts.barcode_parser import parse as parse_barcode
            parsed_barcode = parse_barcode(scan_code)
            if scan_code:
                self._log(f"2D Scan Code: {scan_code}")
                if parsed_barcode:
                    self._log(
                        f"  Supplier Code : {parsed_barcode['supplier_code']}  "
                        f"Part Number: {parsed_barcode['part_number']}  "
                        f"Year/Week: {parsed_barcode['year_week']}  "
                        f"PCB Serial: {parsed_barcode['pcb_serial']}"
                    )
            else:

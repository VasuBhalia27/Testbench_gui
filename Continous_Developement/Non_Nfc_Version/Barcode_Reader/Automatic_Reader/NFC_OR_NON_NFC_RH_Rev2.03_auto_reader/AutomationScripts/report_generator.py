...
def generate_report(
    ...
    parsed_barcode: Optional[Dict[str, str]] = None,
) -> str:
    """
    ...
    parsed_barcode:
        Dictionary of parsed barcode fields (supplier_code, part_number,
        year_week, pcb_serial) as returned by ``barcode_parser.parse()``.
        When provided, these fields are included in the report header.
    ...
    if output_path is None:
        ...
        # Use PCB serial for the filename when available, else fall back to raw scan_code
        serial_part = ""
        if parsed_barcode and parsed_barcode.get("pcb_serial"):
            serial_part = f"_{parsed_barcode['pcb_serial']}"
        elif scan_code:
            serial_part = f"_{scan_code}"
        output_path = os.path.join(
            REPORTS_DIR, f"{ts}{serial_part}_Test_Report_{count_str}_{result_str}.html"
        )

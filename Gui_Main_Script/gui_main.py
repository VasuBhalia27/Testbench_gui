...
from Functional.debugger import *
...
jlink_radio = ttk.Radiobutton(tab2, text="J-Link", variable=debugger_backend_var, value="jlink")
jlink_radio.place(x=230, y=267)

# Connect Debugger Button
canvas2.create_text(61.0, 300.0, anchor="nw", text="Connect Debugger", fill="#FFFFFF", font=("Inter SemiBold", 15 * -1))
...
connect_trace32 = Button(tab2, image=images["tab2_connect_trace32"], 
                         command=lambda: connect_debugger(repo_path_entry, selected_preset, debugger_backend_var, code_status_label), 
                         bd = 0)
connect_trace32.place(x=200, y=300, width=34, height=34)
...
def run_nfc_test():
    t32.SendCmdToDbg("Var.set TestFw_KeepEcuAwake = 1")
    t32.SendDIDGetVal_multiple_entry(nfc_output_variables, nfc_entries, TestFunctionCmd.TEST_GUI_CMD_NFC_TEST_e)
...
def run_can_test():
    try:
        ...
        t32.SendCmdToDbg(f"Var.set TestFw_CanGuiLocalLoopbackEnable = {loopback_value}")

        keep_awake_value = 1 if can_keep_awake_var.get() else 0
        t32.SendCmdToDbg(f"Var.set TestFw_KeepEcuAwake = {keep_awake_value}")

        for idx, entry in enumerate(can_tx_entries):
            value = _parse_u8_from_entry(entry, f"CAN Tx Byte {idx}")
            t32.SendCmdToDbg(f"Var.set DummyBytes.dummy_byte{idx}_U8 = {value}")

        t32.SendDIDGetVal_multiple_entry(can_output_variables, can_entries, TestFunctionCmd.TEST_GUI_CMD_CAN_TEST_e)
...
def run_lin_test():
    try:
        ...
        t32.SendCmdToDbg(f"Var.set TestFw_LinTxPid = {msg_id_value}")

        for idx, entry in enumerate(lin_tx_entries):
            value = _parse_u8_from_entry(entry, f"LIN Tx Byte {idx}")
            t32.SendCmdToDbg(f"Var.set TestFw_LinTxByte{idx} = {value}")

        t32.SendDIDGetVal_multiple_entry(lin_output_variables, lin_entry_list, TestFunctionCmd.TEST_GUI_CMD_LIN_e)
...
def on_closing():
    if messagebox.askokcancel("Quit", "Do you want to quit and disconnect the debugger?"):
        # Stop the polling loop if it exists
        try:
            window.after_cancel(poll_id)
        except:
            pass
        t32.QuitTrace32() # Ensure this function kills the process (see step 3)
        window.destroy()
